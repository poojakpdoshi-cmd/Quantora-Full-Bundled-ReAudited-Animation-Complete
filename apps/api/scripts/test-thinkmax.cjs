...
async function main() {
  ...
  const fallbackResult = await runOptionalThinkMax(
    true,
    { request: 'Create a detailed business website.', plan },
    async () => {
  ...
  assert.strictEqual(fallbackResult.completed, false, 'ThinkMax must report completed: false on provider error');
  assert.deepStrictEqual(fallbackResult.plan, plan, 'ThinkMax must preserve standard plan on provider error');
  assert.strictEqual(fallbackResult.architectureBrief, '', 'ThinkMax must return empty architecture brief on failure');
