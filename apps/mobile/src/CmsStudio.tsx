...
export default function CmsStudio({
  ...
  const [aiDiffModal, setAiDiffModal] = useState<{
    ...
    suggestedSeo?: { title?: string; description?: string };
  } | null>(null);
  const [blueprintProposal, setBlueprintProposal] = useState<CmsBlueprintItem[] | null>(null);
  ...
  async function handleTriggerAiAssist(action: string, targetField: string, currentContent: string) {
    if (!projectId) {
    ...
    try {
      const response = await fetch(
        `${apiBase}/cms/projects/${projectId}/ai-assist?email=${encodeURIComponent(email)}`,
        {
          method: 'POST',
          headers: headers(),
          body: JSON.stringify({
            ...
            tone: 'Luxe & Prestigious'
          })
        }
      );
      const data = await readResponse(response) as { ok: boolean; proposal: any };
      if (data?.proposal) {
        setAiDiffModal({
          targetField,
          original: data.proposal.original,
          proposed: data.proposal.proposed,
          changeSummary: data.proposal.changeSummary,
          suggestedSeo: data.proposal.suggestedSeo
        });
      }
    } catch (assistError) {
      setError(assistError instanceof Error ? assistError.message : 'AI Assistant failed to generate content.');
    } finally {
      setAiLoading(false);
    }
  }
  ...
  async function requestBlueprint(approve = false) {
    if (!projectId) {
      setError('Select a website first.');
      return;
    }
    ...
    try {
      const response = await fetch(
        `${apiBase}/cms/projects/${projectId}/blueprint?email=${encodeURIComponent(email)}`,
        {
          method: 'POST',
          headers: headers(),
      ...
      const data = await readResponse(response) as {
        blueprint?: CmsBlueprintItem[];
        appliedDocumentIds?: string[];
      };
      if (approve) {
        setBlueprintProposal(null);
        setMessage(`${data.appliedDocumentIds?.length || 0} CMS draft documents were created. Existing matching documents were preserved.`);
        await loadCms(projectId);
      } else {
        setBlueprintProposal(Array.isArray(data.blueprint) ? data.blueprint : []);
      }
    } catch (blueprintError) {
      setError(blueprintError instanceof Error ? blueprintError.message : 'Could not generate CMS Blueprint.');
    } finally {
  ...
  return (
    <section className="cms-studio">
      ...
      <div className="cms-project-bar">
        ...
        {!settings ? (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <button
              type="button"
              onClick={() => void requestBlueprint(false)}
      ...
      {blueprintProposal ? (
        <div style={{ background: '#0f172a', border: '1px solid rgba(99,102,241,.45)', borderRadius: 14, padding: 18, marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start' }}>
            <div>
              <h3 style={{ margin: 0 }}>AI CMS Blueprint proposal</h3>
              <p style={{ color: '#94a3b8', fontSize: 13, margin: '6px 0 12px' }}>Review the draft documents below. Nothing has been created until you approve this proposal.</p>
            </div>
            <button type="button" onClick={() => setBlueprintProposal(null)} style={{ background: 'transparent', color: '#94a3b8', border: '1px solid rgba(255,255,255,.2)', borderRadius: 7, padding: '5px 9px', cursor: 'pointer' }}>Close</button>
          </div>
          <div style={{ display: 'grid', gap: 8, marginBottom: 14 }}>
            {blueprintProposal.map((item) => <div key={`${item.collection}:${item.slug}`} style={{ background: '#1e293b', padding: 11, borderRadius: 8 }}><strong>{item.title}</strong><span style={{ color: '#a5b4fc', marginLeft: 8, fontSize: 12 }}>{item.collection}/{item.slug}</span><div style={{ color: '#94a3b8', fontSize: 12, marginTop: 4 }}>{String(item.seo?.description || 'Draft CMS document proposed.')}</div></div>)}
          </div>
          <button type="button" onClick={() => void requestBlueprint(true)} disabled={blueprintLoading || !blueprintProposal.length} style={{ background: '#10b981', border: 0, color: '#06281d', padding: '9px 14px', borderRadius: 8, fontWeight: 700, cursor: 'pointer' }}>{blueprintLoading ? 'Applying…' : 'Approve and create drafts'}</button>
        </div>
      ) : null}
        ...
        >
          ...
          {!settings ? (
            ...
            <>
              ...
              {/* AI Content Assistant Action Bar */}
              <div
                className="cms-ai-assistant-bar"
                style={{
                  display: "flex",
                  alignItems: "center",
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  ...
                  <div>
                    <strong style={{ fontSize: "14px", color: "#c7d2fe", display: "block" }}>
                      AI Content Assistant
                    </strong>
                    <small style={{ color: "#94a3b8", fontSize: "12px" }}>
                      Generate, rewrite, or optimize content with visual diff approval
                    </small>
                  </div>
                </div>
                ...
                <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                  <button
                    ...
                    onClick={() => handleTriggerAiAssist("draft", "body", contentValue("body") || contentValue("description") || title)}
                    disabled={aiLoading}
                    style={{
                      background: "#6366f1",
                      color: "#fff",
                  ...
                  >
                    {aiLoading ? "Thinking…" : "✨ Draft Fresh"}
                  </button>
                  <button
                    type="button"
                    className="small-button"
                    onClick={() => handleTriggerAiAssist("rewrite", "body", contentValue("body") || contentValue("description") || title)}
                    disabled={aiLoading}
                    style={{
                  ...
                  <button
                    type="button"
                    className="small-button"
                    onClick={() => handleTriggerAiAssist("shorten", "body", contentValue("body") || contentValue("description") || title)}
                    disabled={aiLoading}
                    style={{
                      background: "rgba(255,255,255,0.1)",
                      color: "#fff",
                  ...
                  <button
                    type="button"
                    className="small-button"
                    onClick={() => handleTriggerAiAssist("seo", "seo", title)}
                    disabled={aiLoading}
                    style={{
                      background: "rgba(255,255,255,0.1)",
                      color: "#fff",
                  ...
                  <button
                    type="button"
                    className="small-button"
                    onClick={() => handleTriggerAiAssist("faqs", "body", title)}
                    disabled={aiLoading}
                    style={{
                      background: "rgba(255,255,255,0.1)",
                      color: "#fff",
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                💡 <strong>Change Summary:</strong> {aiDiffModal.changeSummary}
              </p>
              ...
              >
                <div>
                  ...
                  >
                    {aiDiffModal.original || (
                      <em style={{ color: "#64748b" }}>Empty field</em>
                    )}
                  </div>
                </div>
            ...
            >
              ...
              <button
                ...
                onClick={() => {
                  updateContentField(
                    aiDiffModal.targetField,
                    aiDiffModal.proposed
                  );
                  if (aiDiffModal.suggestedSeo) {
                    setSeoTitle(aiDiffModal.suggestedSeo.title || "");
                    setSeoDescription(
                      aiDiffModal.suggestedSeo.description || ""
                    );
                  }
                  setMessage(
                    "AI proposal accepted and applied to editor."
                  );
                  setAiDiffModal(null);
                }}
                style={{
