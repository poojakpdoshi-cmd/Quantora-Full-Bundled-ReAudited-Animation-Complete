...
const apiBase = import.meta.env.VITE_API_BASE_URL || (import.meta.env.DEV ? 'http://localhost:8787' : DEFAULT_PUBLIC_API_BASE);
...
function App() {
  ...
  return <main>
    <aside><div className="logo"><img src="/quantora-logo.png" alt="Quantora" /></div><h2>Admin</h2><nav><button type="button" className="active" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>Overview</button><button type="button" onClick={() => document.getElementById('approved-users')?.scrollIntoView({ behavior: 'smooth', block: 'start' })}>Users</button><button type="button" disabled>Offline tokens</button><button type="button" disabled>Templates (later)</button><button type="button" disabled>Deployments (later)</button><button type="button" disabled>Settings (later)</button></nav><button type="button" className="logout" onClick={() => void logout()}>Exit Admin Panel</button><small>Made by Poojak Doshi</small></aside>
    <section className="content"><header><div><p>QUANTORA</p><h1>Control centre</h1></div><div className="header-actions"><button type="button" onClick={() => void loadDashboard()}>Refresh</button><button type="button" className="mobile-logout" onClick={() => void logout()}>Exit Admin Panel</button></div></header>
      ...
      <section className="table"><div className="table-head"><h2>Approve subscriber</h2></div><form className="approve-form" onSubmit={approveUser}><input type="email" value={newUserEmail} onChange={(event) => setNewUserEmail(event.target.value)} placeholder="Subscriber email" required /><input type="datetime-local" value={expiresAt} onChange={(event) => setExpiresAt(event.target.value)} /><input type="number" min="1" max="5" value={maxDevices} onChange={(event) => setMaxDevices(Number(event.target.value))} aria-label="Device limit" title="Device limit" /><input type="number" min="0" max="100" value={dailyWebsiteLimit} onChange={(event) => setDailyWebsiteLimit(Number(event.target.value))} aria-label="Daily website limit" title="Daily website limit" /><button disabled={busy}>Approve email</button></form>{message && <div className="message">{message}</div>}</section>
            <section id="approved-users" className="table"><div className="table-head"><h2>Approved users</h2>
<span>{users.length} loaded</span></div><div className="user-table">{users.length ? users.map((user) => <article key={user.email}><div><strong>{user.email}</strong><span>{user.status} • {user.max_devices} devices • {user.daily_website_limit} website/day</span></div><time>{user.expires_at ? `Expires ${new Date(user.expires_at).toLocaleString()}` : 'No expiry'}</time></article>) : <div className="empty">No approved users yet.</div>}</div></section>
    </section>
  </main>;
}
