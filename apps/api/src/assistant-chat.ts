...
export async function runCmsModel(
  env: AssistantEnv,
  system: string,
  userMessage: string
): Promise<string> {
  ...
  const errors: string[] = [];
  const providers: Array<() => Promise<ProviderReply>> = [];
  if (env.GEMINI_API_KEY) {
    providers.push(() => askGemini(env, system, messages, {
      mode: 'n1',
      useSearch: false,
  ...
  if (env.GROQ_API_KEY) {
    providers.push(() => askGroq(env, system, messages, 1400));
  }
  if (env.AI) {
    providers.push(() => askCloudflare(env, system, messages, 1400));
  }
  if (!providers.length) throw new Error('No CMS AI provider is configured.');
  ...
  throw new Error(`CMS AI providers failed: ${errors.join(' | ')}`);
}
...
export function registerAssistantChatRoutes(
  ...
  app.post('/assistant/chat', async (c: any) => {
    ...
    const configuredProvider = (c.env.QA_PROVIDER || "cloudflare")
      .trim()
      .toLowerCase();
    const errors: string[] = [];
    const processingStartedAt = Date.now();

    const providerMap: Record<string, () => Promise<ProviderReply>> = {
      groq: () => askGroq(c.env, system, messages, route.maxOutputTokens),
      gemini: () => askGemini(c.env, system, messages, route),
      cloudflare: () => askCloudflare(c.env, system, messages, route.maxOutputTokens)
    };

    const defaultOrder = route.mode === 'n1'
      ? ['groq', 'gemini', 'cloudflare']
      : ['gemini', 'groq', 'cloudflare'];

    const providerOrder = (configuredProvider in providerMap)
      ? [configuredProvider, ...defaultOrder.filter((p) => p !== configuredProvider)]
      : defaultOrder;
    ...
    const providers: Array<[
      ...
      .filter((name) => Boolean(providerMap[name]))
      .map((name) => [name, providerMap[name]]);
