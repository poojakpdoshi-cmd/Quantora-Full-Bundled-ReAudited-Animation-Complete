...
function spatialConfigForPlan(plan: WebsitePlan): { dimension: '3D' | '4D' | '5D'; material: string } | null {
  const source = `${plan.businessName} ${plan.websiteType} ${plan.tagline} ${plan.features.join(' ')}`.toLowerCase();
  if (!/\b(?:3d|4d|5d)\b|immersive|spatial|webgl|three[- ]?dimensional|scrollytell/.test(source)) return null;
  const dimension = /\b5d\b/.test(source) ? '5D' : /\b4d\b|scrollytell/.test(source) ? '4D' : '3D';
  const material = /gold|jewel|luxury|metal/.test(source) ? 'gold' : /glass|liquid|frost/.test(source) ? 'glass' : 'hologram';
  return { dimension, material };
}

function createSpatialAppSnippet(config: { dimension: string; material: string }): string {
  return `const spatialConfig = ${JSON.stringify(config)};
function SpatialRuntime() {
  ...
  useEffect(() => {
    ...
    const draw = (time) => {
      ...
      const gradient = context.createRadialGradient(width * (.48 + tilt * .08), height * .4, 4, width * .5, height * .5, Math.max(width, height) * .8);
      gradient.addColorStop(0, spatialConfig.material === 'gold' ? '#fbbf24' : spatialConfig.material === 'glass' ? '#38bdf8' : '#a78bfa');
      gradient.addColorStop(.42, '#0f172a');
      gradient.addColorStop(1, '#020617');
      context.fillStyle = gradient;
      context.fillRect(0, 0, width, height);
      for (let index = 0; index < 7; index += 1) {
        const x = width * (.5 + Math.sin(t * .32 + index) * (.1 + index * .018) + tilt * .04);
        const y = height * (.5 + Math.cos(t * .24 + index) * (.1 + index * .015));
        const radius = Math.max(8, width * (.015 + index * .004));
        context.beginPath();
        context.arc(x, y, radius, 0, Math.PI * 2);
        context.fillStyle = 'rgba(255,255,255,' + (.05 + index * .012) + ')';
        context.fill();
      }
      frame = requestAnimationFrame(draw);
    };
    const observer = new IntersectionObserver((entries) => { active = entries[0]?.isIntersecting !== false; if (active && !frame) frame = requestAnimationFrame(draw); });
    observer.observe(canvas);
    frame = requestAnimationFrame(draw);
    const onOrientation = (event) => setTilt(Math.max(-1, Math.min(1, Number(event.gamma || 0) / 45)));
    if (${JSON.stringify(config.dimension)} === '5D') window.addEventListener('deviceorientation', onOrientation);
    return () => { cancelAnimationFrame(frame); observer.disconnect(); window.removeEventListener('deviceorientation', onOrientation); };
  }, []);
  return <section className="spatial-runtime" aria-label="Interactive spatial preview"><canvas ref={canvasRef} /><div><span>{spatialConfig.dimension} spatial layer</span><strong>Procedural ${config.material} atmosphere</strong><small>Adaptive rendering · reduced-motion safe · non-spatial content remains available</small></div></section>;
}
`;
}

function createSpatialPreviewSnippet(config: { dimension: string; material: string }): { markup: string; script: string } {
  const configJson = JSON.stringify(config);
  return {
    markup: `<section class="spatial-runtime" aria-label="Interactive spatial preview"><canvas id="spatial-canvas"></canvas><div><span>${escapeHtml(config.dimension)} spatial layer</span><strong>Procedural ${escapeHtml(config.material)} atmosphere</strong><small>Adaptive rendering · reduced-motion safe · non-spatial content remains available</small></div></section>`,
    script: `<script>(function(){const c=document.getElementById('spatial-canvas');if(!c)return;const x=c.getContext('2d');const cfg=${configJson};let f=0,active=true,start=performance.now();const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;const draw=function(now){if(!active){f=0;return;}const r=c.getBoundingClientRect(),d=Math.min(devicePixelRatio||1,1.5);c.width=Math.max(1,r.width*d);c.height=Math.max(1,r.height*d);const t=reduced?0:(now-start)/1000,g=x.createRadialGradient(c.width*.5,c.height*.4,4,c.width*.5,c.height*.5,Math.max(c.width,c.height)*.8);g.addColorStop(0,cfg.material==='gold'?'#fbbf24':cfg.material==='glass'?'#38bdf8':'#a78bfa');g.addColorStop(.45,'#0f172a');g.addColorStop(1,'#020617');x.fillStyle=g;x.fillRect(0,0,c.width,c.height);for(let i=0;i<7;i++){x.beginPath();x.arc(c.width*(.5+Math.sin(t*.32+i)*(.1+i*.018)),c.height*(.5+Math.cos(t*.24+i)*(.1+i*.015)),Math.max(8,c.width*(.015+i*.004)),0,Math.PI*2);x.fillStyle='rgba(255,255,255,'+(.05+i*.012)+')';x.fill()}f=requestAnimationFrame(draw)};const o=new IntersectionObserver(e=>{active=e[0]?.isIntersecting!==false;if(active&&!f)f=requestAnimationFrame(draw)});o.observe(c);f=requestAnimationFrame(draw)})()</script>`
  };
}
...
function createAppSource(plan: WebsitePlan, options: ProjectBuildOptions, profile: DesignProfile): string {
  ...
  const spatial = spatialConfigForPlan(plan);
  const spatialSnippet = spatial ? createSpatialAppSnippet(spatial) : '';
...
function createPreviewHtml(plan: WebsitePlan, options: ProjectBuildOptions, profile: DesignProfile): string {
  ...
  const spatial = spatialConfigForPlan(plan);
  const spatialPreview = spatial ? createSpatialPreviewSnippet(spatial) : { markup: '', script: '' };
  const phone = plan.contact?.phone?.replace(/[^0-9]/g, '') || '';
  const formUrl = options.formApiBase && options.formPublicKey
    ? `${options.formApiBase.replace(/\/$/, '')}/public/forms/${options.formPublicKey}/submit`
    : '';
