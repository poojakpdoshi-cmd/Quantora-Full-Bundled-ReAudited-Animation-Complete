...
import { loginAdmin, sendEmailOtp } from '../apps/mobile/src/auth-routing';
...
globalThis.fetch = async (input, init) => {
  ...
  if (url.endsWith('/auth/otp/send')) {
    return Response.json({
      ok: true,
      message: 'Test OTP dispatch accepted.',
      expiresInMinutes: 10
    });
  }
...
async function main() {
  try {
    await sendEmailOtp('https://api.test', {
    ...
    assert.deepEqual(
      ...
      [{ url: 'https://api.test/auth/otp/send', method: 'POST' }],
      'subscriber login calls only POST /auth/otp/send'
    );
    ...
    const standaloneAdminSource = readFileSync(
      resolve(root, 'apps/admin/src/main.tsx'),
      'utf8'
    );
    ...
    assert.match(standaloneAdminSource, />Offline tokens<\/button>/);
    assert.doesNotMatch(standaloneAdminSource, />Payments \(later\)<\/button>/);
  } finally {
