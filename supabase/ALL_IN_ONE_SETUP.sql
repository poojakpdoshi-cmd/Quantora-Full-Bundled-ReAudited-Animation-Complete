-- Quantora V3: run this entire file once in Supabase SQL Editor.
-- Generated from the core schema and compatibility additions.
...
create table if not exists encrypted_provider_credentials (
  connection_id uuid primary key references backend_connections(id) on delete cascade,
...
alter table search_console_oauth_states
  add column if not exists project_id uuid;

alter table search_console_oauth_states enable row level security;
...
create index if not exists idx_search_console_oauth_states_owner on search_console_oauth_states(owner_email, expires_at);
...
create index if not exists idx_backend_connections_owner on backend_connections(owner_email, provider);
