...
import { generateAiCmsProposalWithModel } from './cms-service';
...
export function registerCmsRoutes(
  ...
  app.post(
    '/cms/projects/:projectId/ai-assist',
    async (context: any) => {
      ...
      if (!parsed.success) {
        return context.json({ error: 'Valid CMS access details are required.' }, 400);
      }

      const body = await context.req.json().catch(() => ({}));
      const action = body.action || 'draft';
      const targetField = body.targetField || 'content';
      const currentContent = body.currentContent || '';
      const tone = body.tone || 'Luxe & Prestigious';
      ...
      const projectName = String(auth.project.name || 'Website');

      let proposal;
      try {
        proposal = await generateAiCmsProposalWithModel(context.env, {
          action,
          targetField,
          currentContent,
          tone,
          businessContext: {
            name: projectName,
            industry: (auth.project as any).website_type || 'Services'
          }
        });
      } catch (aiError) {
        console.error('CMS AI provider unavailable or returned invalid output', aiError);
        return context.json({ error: 'CMS AI is unavailable. Configure a supported server-side AI provider and try again.' }, 503);
      }

      return context.json({ ok: true, proposal });
    }
  );

  app.post(
    '/cms/projects/:projectId/blueprint',
    async (context: any) => {
      const parsed = z.object({
        projectId: z.string().uuid(),
        email: z.string().email(),
        ...
        email: context.req.query('email'),
        installationId: context.req.header('X-Device-Id'),
        approve: (await context.req.json().catch(() => ({}))).approve
      });

      if (!parsed.success) {
        return context.json({ error: 'Valid CMS blueprint access details are required.' }, 400);
      }
      ...
      const projectName = String(auth.project.name || 'Website');
      const projectPlan = auth.project.plan && typeof auth.project.plan === 'object'
        ? auth.project.plan as Record<string, unknown>
        : {};
      const industry = String(projectPlan.industry || projectPlan.websiteType || 'Services');
      ...
      const blueprint = [
        {
          ...
          slug: 'home',
          title: 'Home',
          content: { heading: projectName, description: `Welcome to ${projectName}.`, industry },
          seo: { title: `${projectName} | ${industry}`, description: `Discover ${projectName} and its ${industry.toLowerCase()} services.` }
        },
        {
          collection: 'services' as const,
          slug: 'services',
          title: `${industry} Services`,
          content: { heading: `${projectName} Services`, description: `Explore the services offered by ${projectName}.` },
          seo: { title: `${projectName} Services`, description: `Explore ${industry.toLowerCase()} services from ${projectName}.` }
        },
        {
          ...
          content: { questions: [
            { question: `What does ${projectName} offer?`, answer: `${projectName} provides trusted ${industry.toLowerCase()} services.` },
            { question: 'How can I get started?', answer: 'Use the contact form to request more information.' }
          ] },
          seo: { title: `${projectName} FAQ`, description: `Frequently asked questions about ${projectName}.` }
        },
        {
          collection: 'navigation' as const,
      ...
      if (!parsed.data.approve) {
        return context.json({ ok: true, approved: false, blueprint, note: 'Review this proposal and approve it to create draft CMS documents.' });
      }
      ...
      for (const item of blueprint) {
        ...
        const { data: document, error } = await auth.supabase
          ...
          .insert({
            ...
            seo: item.seo,
            sort_order: applied.length * 10
          })
          .select('*')
          .single();

        if (error || !document) {
          return context.json({ error: 'Could not apply the CMS blueprint.' }, 500);
        }

        await saveRevision(auth.supabase, document, email, 'Approved CMS Blueprint');
        applied.push(document.id);
      }

      await increaseContentVersion(auth.supabase, parsed.data.projectId);
      return context.json({ ok: true, approved: true, appliedDocumentIds: applied, blueprint });
    }
  );
